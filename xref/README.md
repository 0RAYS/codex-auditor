...
`xref index` 默认会在 workspace、`build/`、`out/` 等常见位置查找 `compile_commands.json`，也可以显式指定：

```sh
xref -w /目标/路径 \
  -d xref.db \
  index \
  -c /目标/路径/build/compile_commands.json
```
...
```sh
xref -w /目标/路径 meta
xref -w /目标/路径 symbols parse_header
xref -w /目标/路径 def parse_header
xref -w /目标/路径 refs 'c:@F@parse_header'
xref -w /目标/路径 context src/foo.c:120-150
xref -w /目标/路径 context parse_header
xref -w /目标/路径 commits parser -l 20
```
